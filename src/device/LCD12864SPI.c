...
char tab_SPI1[17]={"yumin           "};
char tab_SPI2[17]={"jinlijian       "};
char tab_SPI3[17]={"0123456789      "};
...
//
//#define CPU_F ((double)16000000)
//#define delay_ns(x) __delay_cycles((long)(CPU_F*(double)x/1000000000.0))
//#define delay_us(x) __delay_cycles((long)(CPU_F*(double)x/1000000.0))
//#define Delay_Nms(x) __delay_cycles((long)(CPU_F*(double)x/1000.0))
//
//
...
//#define LCD_RS_H      P8OUT|=BIT1      //P2.0高
//#define LCD_RS_L      P8OUT&=~BIT1     //P2.0低
//#define LCD_RW_H      P8OUT|=BIT2      //P2.1高
//#define LCD_RW_L      P8OUT&=~BIT2     //P2.1低
//#define LCD_EN_H      P7OUT|=BIT0      //2.2高
//#define LCD_EN_L      P7OUT&=~BIT0     //P2.2低
//
//uchar tab_SPI1[16];
//uchar tab_SPI2[16];
//uchar tab_SPI3[16];
...
//  unsigned char GDRAM_hbit,GDRAM_lbit;
//
//  x_Dyte = x;//坐标变换，将坐标原点变换到显示屏的左下角
//  x = 63-y;
//  y = x_Dyte;
//
//  Write_Cmd(0x36); //扩展指令命令
///***X,Y 坐标互换，即普通的X,Y 坐标***/
//  y_Dyte=y/16; //计算在16 个字节中的哪一个
//  y_byte=y&0x0f; //计算在该字节中的哪一位
//  x_Dyte=x/32; //0 为上半屏，1 为下半屏
//  x_byte=x&0x1f; //计算在0~31 当中的哪一行
//
//  Write_Cmd(0x80+x_byte); //设定行地址(y 坐标),即是垂直地址
//  Write_Cmd(0x80+y_Dyte+8*x_Dyte); //设定列地址(x 坐标)，并通过8*y_Dyte 选定上下屏，即是水平地址
//
//  read_data(); //预读取数据
//  GDRAM_hbit=read_data(); //读取当前显示高8 位数据
//  GDRAM_lbit=read_data(); //读取当前显示低8 位数据
//  delay_ns(15);
//
//  Write_Cmd(0x80+x_byte); //设定行地址(y 坐标)
//  Write_Cmd(0x80+y_Dyte+8*x_Dyte); //设定列地址(x 坐标)，并通过8*y_Dyte 选定上下屏
//
//  delay_ns(15);
// if(y_byte<8) //判断其在高8 位，还是在低8 位
// {
//   if(color==1)
//   {
//   Write_Data(GDRAM_hbit|(0x01<<(7-y_byte))); //置位GDRAM 区高8 位数据中相应的点
//   }
//   else
//    Write_Data(GDRAM_hbit&(~(0x01<<(7-y_byte)))); //清除GDRAM 区高8 位数据中相应的点
//
//   Write_Data(GDRAM_lbit); //显示GDRAM 区低8 位数据
// }
// else
...
//  Write_Data(GDRAM_hbit);         //写高8位数据
//  if(color==1)
//   Write_Data(GDRAM_lbit|(0x01<<(15-y_byte))); //置位GDRAM 区高8 位数据中相应的点
//  else
//   Write_Data(GDRAM_lbit&(~(0x01<<(15-y_byte))));//清除GDRAM区高8位数据中相应的点
//  }
//  Write_Cmd(0x30); //恢复到基本指令集
//}
////********************************************************
